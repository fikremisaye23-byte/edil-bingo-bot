...
2. ሁለት ወይም ከዚያ በላይ ተጫዋቾች እኩል ቢያሸንፉ አጠቃላይ ደራሹ ብር ለአሸናፊዎች እኩል ይካፈላል።</div>
            ...
            db.ref('transactions/deposits').on('value', function(snapshot) {
                ...
                listEl.innerHTML = pendingKeys.map(function(key) {
                    ...
                    <div style="background:#1c2640; border:1px solid #2e3b55; border-radius:8px; padding:8px; margin-bottom:8px;">
                        ...
                        ${smsHtml}
                        <div style="font-size:11px; color:#94a3b8; margin-top:6px;">Approve or reject this from the Telegram bot chat.</div>
                    </div>`;
                }).join('');
            });
        ...
        // Deposit approval/rejection intentionally lives ONLY in the Telegram bot
        // (main.py) now -- having a second approve/reject path here caused a
        // wallet-crediting bug, so it was removed on purpose. Use the bot chat
        // to approve or reject deposits.
                ...
                cell.onclick = (function(cellNum, cellElement) {
                    return function() {
                        ...
                        if (index === -1) {
                            ...
                            let canFundCard = (playWalletBalance >= nextCost) || (playWalletBalance === 0 && mainWalletBalance >= nextCost);
                            if (!canFundCard) {
        ...
        function processGameEntry() {
            ...
            // Wallet priority (single source of truth):
            // 1) Play Wallet alone must cover the FULL stake -> deduct from Play only.
            // 2) Otherwise, ONLY if Play Wallet is exactly 0 and Main Wallet alone
            //    covers the FULL stake -> deduct from Main only.
            // 3) Any other case (including "both have some but neither alone is
            //    enough") -> NOT allowed to combine; show Deposit instead.
            let source = null; // 'play' or 'main'
            if (playWalletBalance >= totalDeduction) {
                source = 'play';
            } else if (playWalletBalance === 0 && mainWalletBalance >= totalDeduction) {
                source = 'main';
            }
            ...
            function proceedWithDeduction() {
                if (!db) {
                    if (source === 'play') {
                        playWalletBalance -= totalDeduction;
                    } else {
                        mainWalletBalance -= totalDeduction;
                    }
                    updateWalletDisplay();
                ...
                db.ref('users/' + telegramUserId + '/wallet').transaction(function(current) {
                    ...
                    else if (play === 0 && main >= totalDeduction) actualSource = 'main';
        ...
        function renderDepositApprovedStep(txnKey, record) {
            // NOTE: we intentionally do NOT bump playWalletBalance/depositedBalance
            // or call updateWalletDisplay() here. The bot (main.py) is the only
            // thing that credits the wallet in Firebase now. This function used to
            // also increment the local balance and write it back with a full
            // .set(), which raced against the bot's own write and could overwrite
            // it -- that race is what was causing "approved but balance shows 0".
            // The live wallet listener (loadUserWalletFromFirebase) picks up the
            // bot's Firebase update automatically and refreshes the display.
        ...
        function submitWithdrawRequest() {
            ...
            pushRef.set({
                ...
                applyWalletDelta(-amount, 0, 0, function(ok) {
                    if (!ok) {
                        showToast('⚠️ Could not reserve withdrawal funds.', 4000);
                        return;
                    }
        ...
        function renderWithdrawPendingStep(pushId, amount) {
            ...
            db.ref('transactions/withdrawals/' + pushId).on('value', function(snapshot) {
                ...
                if (record.status === 'approved') {
                    ...
                    db.ref('transactions/withdrawals/' + pushId).off('value');
                    applyWalletDelta(amount, 0, 0, function(ok) {
                        if (ok) addHistoryRecord('withdraw', 'Withdraw Rejected (refunded)', amount);
                    });
                    let modalBodyEl = document.getElementById('modalBodyText');
                    modalBodyEl.innerHTML = `
                        <div style="font-size: 42px; margin-bottom: 4px;">❌</div>
        ...
        function speakNumber(callText) {
            ...
            if ('speechSynthesis' in window) {
                try {
                    ...
                    let spokenText = !isNaN(numPart)
                        ? ((amharicLetterNames[letterPart] || letterPart) + ' ' + numberToWordsAmharic(numPart))
                        : callText.replace('-', ' ');
                    let utterance = new SpeechSynthesisUtterance(spokenText);
                    utterance.lang = 'am-ET';
                    utterance.rate = 0.9;
                    if (cachedVoices.length > 0) {
                        let amVoice = cachedVoices.find(v => v.lang && v.lang.toLowerCase().startsWith('am'));
                        if (amVoice) utterance.voice = amVoice;
                        // If no Amharic voice is installed on this device, we still
                        // send the Amharic text with lang='am-ET' and let the
                        // system's default voice attempt it, rather than silently
                        // falling back to an English voice/reading.
                    }
        ...
        function renderSpectatorCardsUI() {
            ...
            if (!entries.length) {
                container.innerHTML = `
                    <div style="background:var(--card-bg); border-radius:10px; padding:18px 10px; text-align:center; border:1px solid #334155;">
                        <div style="font-size:17px; font-weight:800; color:#fff; margin-bottom:8px;">👀 Watching Only</div>
                        <div style="font-size:12px; color:#cbd5e1; line-height:1.6;">ካርድ ያዙ ተጫዋቾች እስኪገቡ<br>ድረስ እዚህ ይቆዩ።</div>
                    </div>`;
                return;
            }
